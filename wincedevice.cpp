...
WinCEDevice::Ptr WinCEDevice::create(const QString &name,
       Core::Id type, MachineType machineType, Origin origin, Core::Id id)
{
    return Ptr(new WinCEDevice(name, type, machineType, origin, id));
}

QString WinCEDevice::displayType() const
{
    return tr("WinCE");
}

ProjectExplorer::IDeviceWidget *WinCEDevice::createWidget()
{
   qDebug() << "Create widget";
    return 0; //new GenericLinuxDeviceConfigurationWidget(sharedFromThis());
}

QList<Core::Id> WinCEDevice::actionIds() const
{
   return QList<Core::Id>();
    //return QList<Core::Id>() << Core::Id(Constants::GenericDeployKeyToDeviceActionId);
}

QString WinCEDevice::displayNameForActionId(Core::Id actionId) const
{
...
void WinCEDevice::executeAction(Core::Id actionId, QWidget *parent)
{
...
WinCEDevice::WinCEDevice(const QString &name, Core::Id type, MachineType machineType,
...
WinCEDevice::WinCEDevice(const WinCEDevice &other)
    : IDevice(other)
{
}

WinCEDevice::Ptr WinCEDevice::create()
{
    return Ptr(new WinCEDevice);
}

ProjectExplorer::IDevice::Ptr WinCEDevice::clone() const
{
    return Ptr(new WinCEDevice(*this));
}
...
DeviceProcessSignalOperation::Ptr WinCEDevice::signalOperation() const
{
    return DeviceProcessSignalOperation::Ptr(0); //new RemoteLinuxSignalOperation(sshParameters()));
}
